...
def parallel(f, files, frames, path):
    data = ['input file,frames,path,NIQE,time']
...
def main(argv):
    ...
    -h      help
    -i      input video file(s) to be analyzed
            or directory to train NIQE model
    ...
    if input_file == '':
        print(h)
        sys.exit(2)
    # header row and input file, frames, path
    ...
    scores.test(input_file, frames, path)
